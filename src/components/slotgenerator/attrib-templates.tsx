...
const ImmutableTemplateAttributesSlot = () => {
  const {
  ...
  const [selected, setSelected] = useState<ITemplate>(null);

  const data = useCallAPI<ITemplate[]>(
    schema ? GET_COLLECTION_TEMPLATES(collection, schema) : null
  );
...
export default ImmutableTemplateAttributesSlot;
